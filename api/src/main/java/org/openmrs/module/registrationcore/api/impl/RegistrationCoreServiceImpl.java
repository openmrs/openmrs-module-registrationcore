...
public class RegistrationCoreServiceImpl extends BaseOpenmrsService implements RegistrationCoreService, GlobalPropertyListener, ApplicationContextAware {
	...
	/**
	 * @see org.openmrs.module.registrationcore.api.RegistrationCoreService#registerPatient(Patient,
	...
	public List<PatientAndMatchQuality> findPreciseSimilarPatients(Patient patient, Map<String, Object> otherDataPoints,
		...
		List<PatientAndMatchQuality> localMatches = getPreciseSimilarPatientSearchAlgorithm().findSimilarPatients(patient,
		    otherDataPoints, cutoff, maxResults);
		matches.addAll(localMatches);
		
		if (registrationCoreProperties.isMpiEnabled()) {
			List<PatientAndMatchQuality> mpiMatches = registrationCoreProperties.getMpiProvider().findExactMatches(patient,
			    otherDataPoints, cutoff, maxResults);
			matches.addAll(mpiMatches);
	...
	/**
	 * Query to MPI server to find a single patient with Identifier of IdentifierType with
	 * IdentifierTypeUuid provided.
	 * 
	 ...
	 * @should fail if more than one patient exits in the MPI for that specific identifier and
	 *         identifier type
	 */
	@Override
	...
	/**
	 * Serves the PDQ exception if a PDQ error handling service has been specified in the global
	 * properties. Then, throws MPI exception again.
	 * 
